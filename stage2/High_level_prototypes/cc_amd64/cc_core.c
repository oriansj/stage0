...
void function_call(char* s, int bool)
{
	...
	{
		expression();
		emit_out("PUSH_RAX\t#_process_expression1\n");
		passed = 1;
		...
		while(global_token->s[0] == ',')
		{
			global_token = global_token->next;
			expression();
			emit_out("PUSH_RAX\t#_process_expression2\n");
			passed = passed + 1;
		}
	}
	...
	{
		emit_out("POP_RBX\t# _process_expression_locals\n");
	}
...
void function_load(struct token_list* a)
{
	...
	emit_out("LOAD_IMMEDIATE_rax &FUNCTION_");
	emit_out(a->s);
	emit_out("\n");
}
...
{
	current_target = a->type;
	emit_out("LOAD_IMMEDIATE_rax &GLOBAL_");
	emit_out(a->s);
	emit_out("\n");
...
void primary_expr_string()
{
	...
	emit_out("LOAD_IMMEDIATE_rax &STRING_");
...
void common_recursion(FUNCTION f)
{
	...
	global_token = global_token->next;
	emit_out("PUSH_RAX\t#_common_recursion\n");
	f();
	current_target = promote_type(current_target, last_type);
	emit_out("POP_RBX\t# _common_recursion\n");
}
...
void primary_expr()
{
	...
	{
		emit_out("LOAD_IMMEDIATE_rax %0\n");
		common_recursion(primary_expr);
		emit_out("SUBTRACT_rax_from_rbx_into_rbx\nMOVE_rbx_to_rax\n");
	}
	else if('!' == global_token->s[0])
	{
		emit_out("LOAD_IMMEDIATE_rax %1\n");
		common_recursion(postfix_expr);
		emit_out("XOR_rbx_rax_into_rax\n");
	}
	else if(global_token->s[0] == '(')
	{
		global_token = global_token->next;
...
void collect_local()
{
	...
	{
		a->depth = function->locals->depth - 8;
	}
...
void return_result()
{
	...
	{
		emit_out("POP_RBX\t# _return_result_locals\n");
	}
	emit_out("RETURN\n");
}
...
void process_break()
{
	...
	{
		if(NULL == i) break;
		emit_out("POP_RBX\t# break_cleanup_locals\n");
		i = i->next;
	}
	global_token = global_token->next;
	emit_out("JUMP %");
...
void recursive_statement()
{
	...
	if(!match("RETURN\n", out->s))
	{
		...
		{
			emit_out( "POP_RBX\t# _recursive_statement_locals\n");
		}
	}
